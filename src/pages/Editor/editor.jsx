...
import ProblemDescription from "../../components/Admin/EditorComponent/problemDescription";
...
function CodeEditor() {
  ...
  const handleAddFile = () => {
    ...
    let newFileName = prompt(
      `Enter new file name (${
        isCH ? "with .c or .h extension" : `with ${ext}`
      }):`,
  ...
  const checkMainConflict = () => {
    ...
    if (mainCount > 1) {
      alert(
        "Multiple 'main' functions found. Please ensure only one main exists."
      );
      return false;
    }
    return true;
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(
        "http://localhost:5195/api/CodeExecution/run-code",
        {
          method: "POST",
          body: formData,
        }
      ...
      if (response.ok) {
        ...
        if (status === "Failed") {
          ...
          setOutput({
            compilationResult:
              data.compilationResult || "Compilation successful.",
        ...
        setOutput({
          ...
          executionOutput: "",
          errorMessage:
            data.message || "An error occurred while executing the code.",
        });
        setStatusMessage("❌ Execution failed.");
      }
    } catch (error) {
  ...
  return (
  <Split className="flex h-screen" sizes={[35, 65]} minSize={100} gutterSize={4}>
  {/* Pane 1: Description */}
  <ProblemDescription/>
  <Split direction="vertical" className="flex flex-col h-full" sizes={[65, 35]} minSize={100} gutterSize={4}
  >
    {/* Top of Pane 2: Code Editor & Controls */}
    ...
    {/* Bottom of Pane 2: Output Section */}
